...
function generatePlanetsForSingleStar(star, settings) {
	...
	let a_crit = new T.Value(Infinity, T.units.Dist.m);
 ...
 * 
 * @param {T.Value} host_mass	   - Satellite's host body mass (unit: `Mass`).
 * @param {T.Value} companion_mass - Host's companion mass (unit: `Mass`).
 * @param {T.Value} binary_sma	   - Separation between binary components (unit: `Dist`).
 * 
 * @returns {T.Value} Maximal S-type orbit SMA (unit: `Dist`).
 */
export function getMaximalSTypeOrbit(host_mass, companion_mass, binary_sma) {
	const mA = host_mass.as(T.units.Mass.M_Sun);
	const mB = companion_mass.as(T.units.Mass.M_Sun);
	const a_bin = binary_sma.as(T.units.Dist.AU);
	...
	return new T.Value(a_crit, T.units.Dist.AU);
}
 ...
 * 
 * @param {T.Value} mass_greater - Greater binary component's mass (unit: `Mass`).
 * @param {T.Value} mass_lesser  - Lesser binary component's mass (unit: `Mass`).
 * @param {T.Value} binary_sma	 - Separation between binary components (unit: `Dist`).
 * 
 * @returns {T.Value} Minimal P-type orbit SMA (unit: `Dist`).
 */
export function getMinimalPTypeOrbit(mass_greater, mass_lesser, binary_sma) {
	const mA = mass_greater.as(T.units.Mass.M_Sun);
	const mB = mass_lesser.as(T.units.Mass.M_Sun);
	const a_bin = binary_sma.as(T.units.Dist.AU);
...
function generatePlanetsForStar(settings, star, distanceLimit, distanceStart = 0.0, planetsNumber = undefined) {
	...
	for (let planetIndex = 0; planetIndex < planetsToGenerate; planetIndex++) {
		...
		const planet = planetGen.generatePlanet(
			settings, 
			star, 
			new T.Value(sma, T.units.Dist.AU), 
			{ 
				isMoon: false,
				sma_init: sma,
 ...
 * 
 * @param {T.Star} star - Current star. For binaries, amount of P-type orbiting planets is instead calculated from discarded S-type orbiting planets (@see {@link generatePlanetsForBinary}).
...
function getPlanetsNumberToGenerate(star, amountMult) {
	const temperature = star.temperature.as(T.units.Temp.K);
